...
void        process_msdp_varval           ( D_S *d, char *var, char *val );
void        msdp_send_update              ( D_S *d );
...
void        msdp_configure_pluginid       ( D_S *d, int index );

void        write_msdp_to_descriptor      ( D_S *d, char *src, int length );
...
void init_msdp_table(void)
{
	...
	for (index = 0 ; *msdp_table[index].name ; index++)
	{
		if (strcmp(msdp_table[index].name, msdp_table[index+1].name) > 0)
		{
			...
			{
				log_string("\e[31minit_msdp_table: Improperly sorted variable: %s.\e0m", msdp_table[index]);
			}
		}
	}
...
char *msdp_get_var(D_S *d, char *var)
{
	int index;
...
void process_msdp_index_val(D_S *d, int var_index, char *val )
{
	int val_index;
...
void process_msdp_array( D_S *d, int var_index, char *val)
{
...
void msdp_command_list(D_S *d, int index)
{
	char *ptr, buf[MAX_OUTPUT];
	int flag;
...
void msdp_command_report(D_S *d, int index)
{
...
void msdp_command_reset(D_S *d, int index)
{
	int flag;
...
void msdp_command_send(D_S *d, int index)
{
...
void msdp_command_unreport(D_S *d, int index)
{
	if (!HAS_BIT(msdp_table[index].flags, MSDP_FLAG_REPORTABLE))
	{
		return;
